...
#mycursor.execute("CREATE TABLE Reclamations(id int AUTO_INCREMENT PRIMARY KEY, description varchar(255) NOT NULL , rib int(16) NOT NULL , status varchar(255) DEFAULT 'In progress' , ref_code int NOT NULL)")
...
def check_earnings(account_id):
  sql = 'SELECT date , amount_of_transaction , transaction_type , C.full_name from Transactions T , Customers C , Account A where T.account_id='+str(account_id)+' and A.id = T.external_account_id and C.id = A.customer_id LIMIt 5'
  mycursor.execute(sql)
  return mycursor.fetchall()

print(check_earnings(1))
...
def transfer_money(rib , amount, account_id):
    ...
    mycursor.execute(sql)
    f= mycursor.fetchall()[0][0]
    sql = 'SELECT id from Account where RIB='+str(rib)
    mycursor.execute(sql)
    f1 = mycursor.fetchall()[0][0]
    bal2 = int(f)+amount
    external_account_id=int(f1)
    sql = 'UPDATE Account set balance='+str(bal2)+' where RIB='+str(rib)
    mycursor.execute(sql)
    print(bal,bal2)
    datee=  datetime.datetime.now()
    sql = 'INSERT INTO Transactions(date, amount_of_transaction, transaction_type, account_id, external_account_id) VALUES ("{0}","{1}", "{2}", "{3}", "{4}")'.format(datee, amount, "credit", account_id, external_account_id)
    mycursor.execute(sql)

    sql = 'INSERT INTO Transactions(date, amount_of_transaction, transaction_type, account_id, external_account_id) VALUES ("{0}","{1}", "{2}", "{3}", "{4}")'.format(datee, amount, "debit",  external_account_id,account_id)
    mycursor.execute(sql)
    return 'money sent successfully'


#print(transfer_money(888555660,1200.500,1))
...
def cheque_request_status(request_num):
  sql = 'SELECT etat FROM Cheque where demande='+str(request_num)
  mycursor.execute(sql)
  return mycursor.fetchall()[0][0]
